...
public class FloorServiceValidator {
	...
	public void ensureNoPendingRequestForSelf(Long userId, RequestType type) {
		if (floorRepository.existsPendingRequest(userId, type).orElse(false)) {
			...
			if (type == RequestType.JOIN) {
				throw new ResourceException(HttpStatus.BAD_REQUEST, ErrorMessage.PENDING_REQUEST_EXISTS);
			}
			throw new ResourceException(HttpStatus.BAD_REQUEST, ErrorMessage.USER_PENDING_REQUEST);
		}
	...
	public void ensureNoPendingRequestForOther(Long userId, RequestType type) {
		if (floorRepository.existsPendingRequest(userId, type).orElse(false)) {
			...
			throw new ResourceException(HttpStatus.BAD_REQUEST, ErrorMessage.PERSON_PENDING_REQUEST);
		}
	...
	public void ensureRoomAvailable(RoomStatus status) {
		if (status != RoomStatus.AVAILABLE) {
			...
			throw new ResourceException(HttpStatus.BAD_REQUEST, ErrorMessage.ROOM_ALREADY_RESERVED);
		}
	...
	public void ensureRoomMovable(RoomStatus status) {
		if (status == RoomStatus.RESERVED) {
			...
			throw new ResourceException(HttpStatus.BAD_REQUEST, ErrorMessage.ROOM_MOVE_NOT_ALLOWED);
		}
