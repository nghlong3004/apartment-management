...
public class UserServiceValidator {
	...
	public void ensureCanUpdateUser(Long targetUserId) {
		...
		if (!(isAdmin || isSelf)) {
			log.warn("Update user forbidden: actorId={}, targetUserId={}, isAdmin={}, isSelf={}", actorId, targetUserId,
					isAdmin, isSelf);
			throw new ResourceException(HttpStatus.FORBIDDEN, ErrorMessage.PROFILE_UPDATE_FORBIDDEN);
		}
	}
	...
	public void validateCredentials(String rawPassword, User user) {
		if (!passwordEncoder.matches(rawPassword, user.getPassword())) {
			...
			throw new ResourceException(HttpStatus.BAD_REQUEST, ErrorMessage.INVALID_CREDENTIALS);
		}
		if (user.getStatus() != UserStatus.ACTIVE) {
			log.warn("Login failed: account inactive email={}", user.getEmail());
			throw new ResourceException(HttpStatus.FORBIDDEN, ErrorMessage.ACCOUNT_INACTIVE);
		}
	}
